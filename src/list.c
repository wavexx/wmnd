...
/*
 * Return the Nth element of LIST, where N count from zero.  If N larger
 * than the list length, NULL is returned 
 */
 ...
 */
/*
 * INLINE void list_remove_elem(LinkedList** list, void* elem) { while
 * (*list) { if ((*list)->head == elem) list_remove_head(list); *list =
 * (*list ? (*list)->tail : NULL); } }
 */
